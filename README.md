...
- **电子书阅读** - 支持 TXT / EPUB / PDF 三种格式在线阅读：书架管理与自定义分类、单页/双页与滚动/翻页四种阅读版式、字体字号主题行距等阅读设置、自由书签与 3 槽自动进度、IndexedDB 本地缓存（首次打开后免重新下载）；TXT 自动识别 GBK/GB18030 等编码并支持 100MB+ 大文件流式章节索引
...
1. 从 [Release](../../releases) 下载对应系统版本（目前仅提供 Windows/Linux x86\_64 预编译版本）
2. 解压后直接启动可执行文件
...
> **注意**：`argon2` 参数影响用户登录密码和备份加密密码的哈希强度。修改这些参数只对此后新设置的登录密码和新创建的加密备份生效；已有登录密码的哈希参数内嵌在哈希值中、已有备份的 KDF 参数记录在备份的 `.info` 文件中，验证与恢复时各自读取所存储的参数，因此修改配置后仍可正常登录和恢复旧备份。在低性能设备（如路由器）上，可降低 `m_cost`（如 `4096`）和 `t_cost`（如 `1`）以减少内存占用和登录耗时，但会降低密码抗暴力破解的安全性。
