...
**路径**：GET /ws/note?notebook_id={id}&path={note_relative_path}

**功能**：基于 Yjs CRDT 的实时协作编辑。非加密笔记本打开笔记时自动建立连接，多用户同时编辑同一笔记时互不覆盖。

**认证**：通过 session_id cookie 认证（与 HTTP 接口相同）。

**限制**：加密笔记本不支持协作（返回 403），仍使用手动保存模式。

**协议**：全二进制消息。

**服务端 → 客户端**：

连接建立后，服务端立即下发存储的 Yjs 全量快照（二进制）。若无快照则不发送任何消息。

之后实时转发其他客户端的 Yjs 增量更新（原始二进制，无类型前缀）。

**客户端 → 服务端**：

| 首字节 | 含义 | 载荷 |
|--------|------|------|
| `0x00` | Yjs 增量更新 | Yjs update 二进制（服务端转发给房间其他客户端） |
| `0x01` | 持久化快照 | JSON：`{ "yjs_state": "base64编码的Yjs全量state", "text": "markdown纯文本" }` |

**持久化行为**：

- 收到 `0x01` 快照时，服务端将 yjs_state 解码写入 `{note_path}.yjs` 伴随文件，将 text 写入原 `.md` 文件
- 客户端在文档变更后 2 秒防抖发送一次快照
- 新客户端连接时收到 `.yjs` 文件内容，apply 后得到完整文档

**房间管理**：

- 房间按笔记文件的物理路径（canonicalize）划分
- 同一物理路径的所有连接共享一个房间
- 最后一个连接断开时房间自动清理
