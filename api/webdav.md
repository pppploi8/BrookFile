...
**路径**：POST /api/webdav/cors/list

**功能**：获取当前用户配置的 WebDAV 跨域访问域名白名单。需要登录后才能访问。
...
{
  "success": true,
  "origins": ["https://example.com", "http://192.168.1.10:8080"]
}
```

| 字段 | 类型 | 说明 |
|-----|------|------|
| origins | array | 当前用户配置的域名白名单列表 |
...
**路径**：POST /api/webdav/cors/save

**功能**：整组保存当前用户的 WebDAV 跨域访问域名白名单（覆盖式替换）。需要登录后才能访问。
...
| 字段 | 类型 | 必填 | 说明 |
|-----|------|------|------|
| origins | array | 是 | 域名白名单列表，每项格式为 http(s)://主机[:端口]，不含路径。传空数组表示清空白名单 |
...
|---------|------|
| `NOT_LOGGED_IN` | 用户未登录 |
| `CORS_ORIGIN_INVALID` | 存在格式无效的域名 |

**处理流程**：
1. 逐项校验域名格式（http/https 协议 + 主机 + 可选端口，不含路径），自动去除首尾空白与结尾斜杠、去重
2. 事务内删除该用户原有白名单，插入新列表
3. 刷新服务端跨域白名单内存缓存

**说明**：白名单仅控制浏览器是否允许对应域名发起跨域请求，不绕过 WebDAV 认证；实际读写仍需提供正确的账号密码。服务端对所有用户的白名单取并集判断 `Origin` 是否放行。
